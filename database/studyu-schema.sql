...
SELECT TO authenticated USING (((bucket_id = 'observations'::text) AND (owner = auth.uid())));
...
CREATE
POLICY "Allow Researchers to view observations of own studies" ON storage.objects FOR
SELECT TO public USING (((bucket_id = 'observations'::text) AND
