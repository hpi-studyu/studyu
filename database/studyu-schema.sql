...
CREATE POLICY "Editors can do everything with study invite codes if only participation is invite" ON public.study_invite USING (( SELECT public.can_edit(auth.uid(), study.*) AS can_edit
   FROM public.study
