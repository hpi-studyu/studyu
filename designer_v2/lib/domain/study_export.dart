...
/*
  List<Map<String, dynamic>> get measurementsExportData {
    final List<Map<String, dynamic>> data = [];
    if (participantsProgress == null || participants == null) {
      return data;
    }
    ...
    for (var i = 0; i < observations.length; i++) {
      ...
      surveyColumns['survey${i}_id'] = surveyMeasurement.id;
      surveyColumns['survey${i}_name'] = surveyMeasurement.title;
      surveyColumns['survey${i}_answered'] = false;
      surveyAnsweredColumnById[surveyMeasurement.id] = 'survey${i}_answered';
    ...
    for (final record in participantsProgress!) {
      if (!MeasurementResultTypes.values.contains(record.resultType)) {
        continue; // skip non-measurement progress records
      }

      final measurement =
          observations.firstWhereOrNull((o) => o.id == record.taskId);
      final intervention =
          interventions.firstWhereOrNull((e) => e.id == record.interventionId);

      final Map<String, dynamic> row = {
        'participant_id': record.subjectId,
        'participant_started_at': record.startedAt!,
        ...
        'measurement_id': record.taskId,
        'measurement_name': measurement?.title ?? invalidKey,
        'measurement_time': record.completedAt!,
        ...surveyColumns,
      };
  ...
  Map<String, dynamic> get interventionsExportData {
    return {};
  }

  Map<String, dynamic> get participantsExportData {
    return {};
  }
   */
}
