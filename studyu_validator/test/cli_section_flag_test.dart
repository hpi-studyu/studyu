  ...
  group('validateSection', () {
    test('section=study_info runs only study info checks', () {
      final r = validateSection(
          _baseStudyJson, 'study_info', ValidationLevel.draft);
      expect(r, isNotNull);
      // title is set in the JSON, so should pass
      expect(r!.errors.where((e) => e.code == 'study_info.title_required'),
          isEmpty);
    });

    test('section=consent at publish with empty consent -> consent.no_items',
        () {
      final r = validateSection(
          _emptyConsentStudyJson, 'consent', ValidationLevel.publish);
      expect(r, isNotNull);
    ...
    test(
        ...
        () {
      final r = validateSection(
          _dupObsStudyJson, 'observations', ValidationLevel.draft);
      expect(r, isNotNull);
      expect(r!.valid, isFalse);
      expect(
          r.errors.any(
    ...
    test('unknown section string returns null from validateSection', () {
      final r = validateSection(
          _baseStudyJson, 'unknown_section', ValidationLevel.draft);
      expect(r, isNull);
    });

    test('section=interventions passes for empty interventions at draft', () {
      final r = validateSection(
          _baseStudyJson, 'interventions', ValidationLevel.draft);
      expect(r, isNotNull);
    ...
    test('section=schedule passes for valid schedule', () {
      final r = validateSection(
          _baseStudyJson, 'schedule', ValidationLevel.draft);
      expect(r, isNotNull);
      expect(r!.valid, isTrue);
    });
    ...
    test('section=questionnaire passes for empty questionnaire', () {
      final r = validateSection(
          _baseStudyJson, 'questionnaire', ValidationLevel.draft);
      expect(r, isNotNull);
      expect(r!.valid, isTrue);
    });
    ...
    test('section=report passes for empty report', () {
      final r = validateSection(
          _baseStudyJson, 'report', ValidationLevel.draft);
      expect(r, isNotNull);
      expect(r!.valid, isTrue);
    });
    ...
    test('section=eligibility passes for empty eligibility', () {
      final r = validateSection(
          _baseStudyJson, 'eligibility', ValidationLevel.draft);
      expect(r, isNotNull);
      expect(r!.valid, isTrue);
    });
  });
